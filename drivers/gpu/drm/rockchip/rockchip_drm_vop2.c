...
static int vop2_crtc_debugfs_init(struct drm_minor *minor, struct drm_crtc *crtc)
{
	...
	drm_debugfs_vop_color_bar_add(crtc, vop2->debugfs);
#endif
...
static void vop2_crtc_atomic_begin(struct drm_crtc *crtc, struct drm_crtc_state *old_crtc_state)
{
	...
	drm_atomic_crtc_for_each_plane(plane, crtc) {
		...
		nr_layers++;
		rockchip_drm_dbg(vop2->dev, VOP_DEBUG_PLANE, "%s active zpos:%d for vp%d from vp%d\n",
			     win->name, vpstate->zpos, vp->id, old_vp->id);
	}

	rockchip_drm_dbg(vop2->dev, VOP_DEBUG_PLANE, "vp%d: %d windows, active layers %d\n",
