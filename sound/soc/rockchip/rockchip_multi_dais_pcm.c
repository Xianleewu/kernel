...
static snd_pcm_uframes_t dmaengine_mpcm_raw_pointer(struct snd_pcm_substream *substream)
{
	struct dmaengine_mpcm_runtime_data *prtd = substream_to_prtd(substream);
...
static int dmaengine_mpcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
...
static int dmaengine_mpcm_ioctl(struct snd_pcm_substream *substream,
				unsigned int cmd, void *arg)
{
	return snd_pcm_lib_ioctl(substream, cmd, arg);
}

static int dmaengine_mpcm_hw_free(struct snd_pcm_substream *substream)
{
...
static int dmaengine_mpcm_copy_user(struct snd_pcm_substream *substream,
...
static int dmaengine_mpcm_prepare(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtd, SND_DMAENGINE_MPCM_DRV_NAME);
					 ...
					 struct snd_pcm_substream *substream)
{
	...
	if (strstr(dev_driver_string(component->dev), "i2s")) {
		...
		tx = snd_soc_component_read32(component, I2S_TXFIFOLR);
		rx = snd_soc_component_read32(component, I2S_RXFIFOLR);
